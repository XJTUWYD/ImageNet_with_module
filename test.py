...
from models import AlexNet_dorefa
import numpy as np
...
def test( 
	...
	test_images = sorted(os.listdir(os.path.join(imagenet_path, '/data2/ILSVRC2012/val')))
	test_labels = tu.read_test_labels(os.path.join(imagenet_path, '/data2/ILSVRC2012/ILSVRC2012_devkit_t12/data/ILSVRC2012_validation_ground_truth.txt'))
	...
	_, pred = AlexNet_dorefa.classifier(x, 1.0,is_training = False)
	...
	with tf.Session(config=tf.ConfigProto()) as sess:
		...
		for i in range(test_examples):
			...
			image_patches = tu.read_k_patches(os.path.join(imagenet_path, '/data2/ILSVRC2012/ILSVRC2012_img_val', test_images[i]), k_patches)
			label = test_labels[i]
